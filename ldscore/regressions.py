...
class Hsq(LD_Score_Regression):
    ...
    def __init__(self, y, x, w, N, M, n_blocks=200, intercept=None, slow=False, 
            ...
            chr=None, verbose=True,            
            approx_ridge=False, ridge=False, ridge_lambda=None, standardize_ridge=True,
            nonneg_constraints=None):
        step1_ii = None
    ...
    def _overlap_output(self, category_names, overlap_matrix, M_annot, M_tot, print_coefficients):
        ...
        diff_p = [np.nan if diff_se[i]==0 else 2*tdist.sf(abs(diff_est[i]/diff_se[i]), self.jknife.delete_values.shape[0]) \
            for i in range(self.n_annot)]
        
        zscore = np.zeros(self.n_annot)
        ...
        coef_pvalue = chi2(1).sf(zscore**2)
        
        #import ipdb; ipdb.set_trace()
        # stat_zero_h2_nosize = self.cat[0,:] / self.cat_se
        # pvalue_zero_h2_nosize = 2*tdist.sf(np.abs(stat_zero_h2_nosize), self.jknife.delete_values.shape[0])
        
        pvalue_zero_enr = [np.nan if prop_hsq_overlap_se[0,i]==0 else 2*tdist.sf(abs(prop_hsq_overlap[0,i]/prop_hsq_overlap_se[0,i]), self.jknife.delete_values.shape[0]) \
            for i in range(self.n_annot)]
        
        h2_est = overlap_matrix.dot(self.coef)
        se_stat_zero_h2 = np.sqrt(np.diag(overlap_matrix.dot(self.coef_cov).dot(overlap_matrix.T)))
        #import ipdb; ipdb.set_trace()
        pvalue_zero_h2 = [np.nan if se_stat_zero_h2[i]==0 else 2*tdist.sf(abs(h2_est[0,i]/se_stat_zero_h2[i]), self.jknife.delete_values.shape[0]) \
            for i in range(self.n_annot)]
        ...
        df = pd.DataFrame({
            ...
            'Enrichment_stat': one_d_convert(diff_est),
            'Enrichment_stat_std_error': one_d_convert(diff_se),
            'Enrichment_p':diff_p,
            'Coefficient': one_d_convert(self.coef),
            'Coefficient_std_error': self.coef_se,
            'Coefficient_z-score': zscore,
            'Coefficient_p': coef_pvalue,            
            'Zero_enr_p': pvalue_zero_enr,
            'Zero_h2_p': pvalue_zero_h2,            
        ...
        if print_coefficients:
            df = df[['Category', 'Prop._SNPs', 'Prop._h2', 'Prop._h2_std_error',
                    ...
                    #'Enrichment_stat', 'Enrichment_stat_std_error',
                    'Enrichment_p',
                    'Coefficient', 'Coefficient_std_error','Coefficient_z-score', 'Coefficient_p', 'Zero_enr_p', 'Zero_h2_p']]
        else:
            df = df[['Category', 'Prop._SNPs', 'Prop._h2', 'Prop._h2_std_error',
                    'Enrichment','Enrichment_std_error', 'Enrichment_stat', 'Enrichment_stat_std_error', 'Enrichment_p']]
        return df
