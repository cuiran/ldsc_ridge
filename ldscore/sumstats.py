...
def _check_variance(log, M_annot, ref_ld):
    ...
    ii = ref_ld.ix[:, 2:].var() == 0  # NB there is a SNP and CHR column here
    if ii.all():
...
def _read_ld_sumstats(args, log, fh, alleles=False, dropna=True):
    ...
    n_annot = len(ref_ld.columns) - 2   #Omer: Changed to -2 because we also have chromosome column now
    ...
    if (args.anno is not None):
        ...
        for anno in annotations:
            if (anno not in ref_ld.columns.str[:-4]):
                raise ValueError('Annotation {A} not found in LD scores file'.format(A=anno))
            cols_to_keep[anno == ref_ld.columns.str[:-4]] = True        
...
def cell_type_specific(args, log):
    ...
    assert not args.force_nonneg, 'force_nonneg not yet supported with cell-type specific annotations'
