...
parser.add_argument('--ridge', default=False, action='store_true', 
    help='Use Ridge regression')
...
parser.add_argument('--force-nonneg', default=False, action='store_true', 
    help='Force use of non-negative constraints. Note that this will use the Ridge regression code even if --ridge was not specified, but the results will not be regularized')
...
if __name__ == '__main__':
    ...
    try:
        ...
        if args.ridge_lambda:
            assert args.ridge, '--ridge_lambda can only be used with --ridge'

        if args.force_nonneg:
            assert not args.ridge, '--force-nonneg and --ridge cannot be used together (until I fix the standardization/intercept thing)'
